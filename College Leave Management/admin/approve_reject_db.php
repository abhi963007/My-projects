...
if ($_SESSION['sid'] == session_id() && $_SESSION['user'] == "Admin") {
    ...
    if (isset($_POST['staff_id']) && isset($_POST['start_date'])) {
        ...
        // Check if the leave is being approved or rejected by the Admin
        if ($leave_status == 'Approved') {
            // Update the Admin approval only
            $sql = "UPDATE leave_requests SET admin_approval = 1 WHERE staff_id = ? AND start_date = ?";
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location: view_leave_requests.php?msg=Admin approved leave.");
